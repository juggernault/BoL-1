...
function OnTick()
    ...
    if AutoLevelspells then
        autoLevelSetSequence({1, 2, 3, 1, 1, 4, 1, 2, 1, 2, 4, 2, 2, 3, 3, 4, 3, 3})
    end
end
...
function OnProcessSpell(unit, spell)
	...
	if unit.isMe and spell.name:lower():find("attack") and Menu.LaneC.clearQ and Menu.LaneC.laneclr then
		SpellTarget = spell.target
			DelayAction(function() CastSpell(_Q, mousePos.x, mousePos.z) end, spell.windUpTime - GetLatency() / 2000)
	end
